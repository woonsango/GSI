...
#define _DUTY_MIN 570 // servo full clockwise position (0 degree)
#define _DUTY_NEU 1485 // servo neutral position (90 degree)
...
void loop() {
  ...
  if (millis() < (last_sampling_time + INTERVAL))  //무슨 의미인지 알기(몇초에 몇번 검사할지 정하는 코드)
    return;
  ...
  if (dist_raw < _DIST_MIN) {
    ...
    dist_prev = dist_raw;
    myservo.writeMicroseconds(servo(dist_ema)) ;
  }
...
float servo(float distance) {
  return ((2400.0-570.0)/(360.0-180.0))*(distance - 180) + 570 ;
}
