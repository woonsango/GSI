...
void loop() {
  ...
  if (distance < _DIST_MIN) {
    distance = b_distance ;    // Set Lower Value
    digitalWrite(PIN_LED, 1);       // LED OFF
  } else if (distance > _DIST_MAX) {
    distance = b_distance ;    // Set Higher Value
    digitalWrite(PIN_LED, 1);       // LED OFF
  } else {    // In desired Range
    b_distance = distance ;
    analogWrite(PIN_LED, Led(distance));       // LED ON      
  }
...
float Led(float distance) {
  float ledValue = 1.0/100.0 ;
  if (distance < 200){
    return float (255.0)*(-(ledValue*distance)+2) ;
